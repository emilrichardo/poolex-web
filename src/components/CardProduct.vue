<template>
  <div @click="tooggleProduct()"
    class="bg-[#EDEEFD] relative rounded-lg overflow-hidden h-24 shadow-lg neomorphism flex items-center transition-all cursor-pointer hover:scale-105">
    <span
    ...
    <div class="card-body py-3 px-6 flex items-center">
      <NuxtImg class="w-8 flex-none mr-2" v-if="icon" :src="icon.data?.attributes?.url" alt="icon_product" />
  ...
  <div v-if="openPanel"
    ...
    <div class="panel-header flex justify-between">
      ...
      <div class="flex gap-4">
        ...
        <Button @click="openPanel = !openPanel" type="button" class="pr-8" variant="default_outline" size="sm">
    ...
    <div class="panel-body text-sm pt-4">
      <div v-if="!isRegister" class="grid lg:grid-cols-3 gap-5">
        <NuxtImg v-if="data?.attributes?.Featured_image?.data" class="hidden md:block rounded-lg w-full"
          :src="data?.attributes?.Featured_image?.data?.attributes?.url" alt="product image" />
        <div class="caption lg:col-span-2 text-lg">
          <p>{{ useLocaleComponent(locale, data).description }}</p>
          <div class="flex gap-4 mt-4">
            <NuxtLink v-if="data?.attributes?.register_link" :to="data?.attributes?.register_link"><Button type="button"
                size="sm" variant="primary_shadow">{{
      ...
      <div v-else>
        <div class="h-full flex items-center justify-center py-4" v-if="loadingData">
        ...
        <div class="dataArray col-span-3 bg-light rounded-md shadow-lg" v-for="plan in currentData?.data_array">
          ...
          <div v-if="planIsOpen === plan.name" class="grid grid-cols-1 lg:grid-cols-3 gap-1 py-2 px-4">
            <template v-for="(value, key) in plan">
              <h5 v-if="key != 'name'" class="text-xs">
                <strong class="font-semibold">{{ convertirCadena(key) }}:
                </strong>
                {{ value }}
              </h5>
            </template>
        ...
        <div v-if="currentData?.success" class="grid grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
