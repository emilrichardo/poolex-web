<template>
  <section :class="`block_text py-24 ${(content.headings?.align === 'center' && 'text-center') ||
    ...
    <div
      ...
      <div v-for="item in memberships" @click="openModal(item.id)"
        ...
        <NuxtImg v-if="item.attributes?.icon" :src="item.attributes?.icon?.data?.attributes?.url"
          :alt="item.attributes?.name" class="w-28 mb-4" />
        <h3 class="text-md lg:text-xl font-semibold text-dark-100" v-if="item.attributes?.name">
