<template>
  <header :class="` ${haveHero ? 'fixed' : 'fixed'
      ...
      <nav class="flex items-center justify-between gap-1 lg:gap-8">
        <div class="brand shrink-0">
          <NuxtLink :to="localePath('/')">
            <NuxtImg v-if="data.header.logo_light.data?.attributes?.url" class="w-32 sm:w-40"
              :src="data.header?.logo_light?.data?.attributes?.url" :alt="data?.Site_name" />
        ...
        <div class="menu_1">
          <ul class="hidden lg:flex flex-row items-center gap-4">
            ...
            ).header?.menu_links" :key="item.key">
              <template v-if="item.active">
                <NuxtLink v-if="item.name !== 'products'" :to="localePath(item.url)" @click="menuIsOpen = false">
                  <Button :type="item.type" :size="item.size" :variant="item.style">{{ item.label }}
                  </Button>
                </NuxtLink>
                <a class="cursor-pointer hover:text-primary" v-else @click="toggleMegaMenu()">{{ item.label }}</a>
              </template>
            </li>
          </ul>
        </div>
          ...
          <ul class="flex items-center md:flex-row gap-4">
            <template v-if="globalOptions.userData">
              <li class="hidden lg:block">
                <Button type="button" variant="default" size="sm" @click="getLogOut()">
                  <span>Logout</span>
                  <Icon class="ml-2" name="heroicons-outline:logout" />
                </Button>
              </li>
              <li class="relative hidden lg:block">
                <Button @click="openBo = !openBo" type="button" variant="primary" size="sm">Backoffices
                ...
                <div class="openbo-overlay h-screen w-screen fixed top-0 left-0 z-0" v-if="openBo"
                  @click="openBo = !openBo"></div>
              </li>
            </template>

            <li v-else class="hidden lg:block">
              <Button @click="toggleModalLogin()" type="button" variant="primary" size="sm" class="flex">Login</Button>
            </li>
            <li class="lg:hidden">
              <Button @click="toggleMenu()" type="button" :variant="menuIsOpen ? 'primary' : 'default'" size="sm">
        ...
        <div :class="`menu_mobile lg:hidden ${menuIsOpen
          ...
          <ul class="flex flex-col items-center mb-3">
            <li v-for="item in useGetLocale(
              ...
              <template v-if="item.active">
                <NuxtLink v-if="item.name !== 'products'" :to="localePath(item.url)" @click="menuIsOpen = false">
                  <Button :type="item.type" :size="item.size" :variant="item.style" class="text-lg py-3 px-7 block">{{
                    item.label }}
                  </Button>
                </NuxtLink>
                <a class="cursor-pointer hover:text-primary text-lg py-3 px-7 block" v-else @click="toggleMegaMenu()">{{
                  item.label }}</a>
              </template>
            </li>
          </ul>
          <ul class="flex flex-col items-center gap-4">
            <template v-if="globalOptions.userData">
              <li>
                <Button type="button" variant="default" size="sm" class="w-60" @click="getLogOut()">
                  <span>Logout</span>
                  <Icon class="ml-2" name="heroicons-outline:logout" />
                </Button>
              </li>
              <li class="relative -order-1">
                <Button @click="openBo = !openBo" type="button" variant="primary" size="sm" class="w-60">Backoffices
                  <Icon class="ml-2" name="heroicons:chevron-down-solid" />
                </Button>
                <div v-if="openBo"
    ...
    <MegaMenu v-if="megaMenuOpen" @toggleMegaMenu="toggleMegaMenu" @closeMenu="closeMenu"></MegaMenu>

    <div @click="menuIsOpen = false" class="ovelay bg-primary opacity-0 -z-30 fixed w-screen h-screen" v-if="menuIsOpen">
      overlay primary
    </div>
  </header>

  <ClientOnly>
    <Login v-if="modalState" @closeMenu="closeMenu" @toggleModalLogin="toggleModalLogin" />
  </ClientOnly>
</template>
<script setup>
